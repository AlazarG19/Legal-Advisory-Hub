...
async def ask_question(query: Query = Body(...)):
    ...
    if not results or results[0][1] < 0.5:  # Adjust the threshold here
        if not results or results[0][1] < 0.3:
            raise HTTPException(status_code=404, detail="No matching context found")
        else:
            # Return the results with the lower confidence threshold
            return results
...
async def import_database():
    try:
        subprocess.run(["python3", "knowledgeBase/create_database.py"])
