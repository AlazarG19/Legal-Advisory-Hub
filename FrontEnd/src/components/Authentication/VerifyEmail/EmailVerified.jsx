...
const CheckEmail = () => {
    ...
    const resendEmail = (event) => {
        ...
        fetch(`http://localhost:3000/getUser/${params.id}`).then(res => res.json()).then(result => {
            console.log(result)
    ...
    useEffect(() => {
        fetch(`http://localhost:3000/verify/${params.id}/${params.token}`).then(res => res.json()).then(result => {
            setloading(false)
