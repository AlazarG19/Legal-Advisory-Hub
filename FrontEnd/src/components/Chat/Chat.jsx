...
function Chat({ socket, username, room, clients }) {
  ...
  useEffect(() => {
    ...
    socket.on("receive_message", fetchMessages);
      ...
      >
        ...
        <div className="card-footer pt-4" id="kt_drawer_chat_messenger_footer">
          ...
          <div className="d-flex flex-stack">
            {/*begin::Send*/}
            {userType == 'freelancer' ?  <>
            <Link to={`/createOffer/${id}`} className="btn btn-primary container-fluid mt-5" > {offerText}</Link>

            
            
            </>  : <Link to={`/ReportFreelancer/${id}`} className="btn btn-danger container-fluid mt-5" > ! Report</Link>}
