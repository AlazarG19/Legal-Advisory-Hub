...
const FinalCustomForm = () => {
    ...
    const handleSubmit = () => {
        ...
        let body = { formid: formid, sections: state.adminsections.value, formtext: "<pre class='pretext'>" + value + "</pre>", formname: formName, formdescription: formDescription, category: category }
        console.log(body)
    ...
    const editQuestion = (idToUpdate) => {
        ...
        const oldadminsections = Array.from(state.adminsections.value)
        const index = oldadminsections.findIndex(item => item.id === idToUpdate);
        oldadminsections[index] = { ...oldadminsections[index], questions: "" }
        store.dispatch(editadminstatement({ adminsection: oldadminsections }))
    }
        ...
        if (state.counter.value == 0) {
            fetch(`http://localhost:3000/api/forms/${id}`, {
                ...
                .then((data) => {
                    ...
                    setadminsections(data[0].sections)
                    ...
                    dispatch(editadminstatement({ adminsection: data[0].sections }))
                    dispatch(increment())
                })
                .catch((error) => {
                    // console.log("error while sending")
                                                                    ...
                                                                    return <div className="d-flex flex-stack position-relative mt-8">
                                                                        ...
                                                                        <div className="fw-semibold ms-5 text-green-600">
                                                                            ...
                                                                            <div className="text-gray-400">
                                                                                <input id={adminsection.id} key={adminsection.id} type="text" onChange={(e) => { handleChange(adminsection.id, e.target.value) }} className="form-control form-control" placeholder="Enter your question" defaultValue={adminsection.questions} name="settings_name" />
                                                                        ...
                                                                        {adminsection.questions != "" ?
                                                                            <button onClick={(e) => { e.preventDefault(); editQuestion(adminsection.id) }} className="btn btn-sm btn-icon btn-clear btn-active-light-primary me-3" data-bs-toggle="tooltip" data-bs-placement="top" title="Reply">
